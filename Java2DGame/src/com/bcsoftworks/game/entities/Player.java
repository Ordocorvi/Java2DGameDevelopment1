package com.bcsoftworks.game.entities;

import com.bcsoftworks.game.InputHandler;
...
public class Player extends Mob {
	...
	public void tick() {
		...
		if (isSwimming && level.getTile(this.x >> 3, this.y >> 3).getId() != 3) {
			//System.out.println("isNotSwimming");
			isSwimming = false;
		}
		tickCount++;
	...
	public void render(Screen screen) {
		...
		screen.render(xOffset - (modifier * flipTop) + modifier, yOffset, xTile + yTile * 32 + 1, color, flipTop, scale);
	...
	public boolean hasCollided(int xa, int ya) {
		...
		for (int x = xMin; x < xMax ; x++) {
			if (isSolidTime(xa, ya, x, yMin)) {
				return true;
			}
		}
		for (int x = xMin; x < xMax ; x++) {
			if (isSolidTime(xa, ya, x, yMax)) {
				return true;
			}
		}
		for (int y = yMin; y < yMax ; y++) {
			if (isSolidTime(xa, ya, xMin, y)) {
				return true;
			}
		}
		for (int y = yMin; y < yMax ; y++) {
			if (isSolidTime(xa, ya, xMax, y)) {
				return true;
			}
		}
